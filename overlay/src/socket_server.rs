...
use crate::protocol::{Snapshot, PROTOCOL_HEADER, MAX_PAYLOAD_SIZE};
...
fn handle_connection(
    ...
    #[cfg(unix)]
    {
        ...
        use libc::{getsockopt, SOL_SOCKET, SO_PEERCRED, ucred};
        ...
        let result = unsafe {
            getsockopt(fd, SOL_SOCKET, SO_PEERCRED, &mut cred as *mut _ as *mut _, &mut cred_len)
        };
        
        if result == 0 {
            let current_uid = unsafe { libc::getuid() };
            if cred.uid != current_uid {
                warn!("Rejected connection from different UID: {} (expected {})", cred.uid, current_uid);
                return Ok(());
            }
